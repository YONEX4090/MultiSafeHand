...
class MACPAO_Trainer:
    ...
    def calculate_imp_weights(self, sample):
        ...
        data = imp_weights, adv_targ_hybrid, factor_batch, active_masks_batch, dist_entropy, aver_episode_costs,\
            cost_adv_targ, values, value_preds_batch, return_batch, cost_values, cost_preds_batch, cost_returns_barch
        return data

    def ppo_update(self, data, imp_weight_list, order, agent_num):
        imp_weights, adv_targ_hybrid, factor_batch, active_masks_batch, dist_entropy, aver_episode_costs,\
            cost_adv_targ, values, value_preds_batch, return_batch, cost_values, cost_preds_batch, cost_returns_barch= data

        k = 0.2
        clip_param = 0.9
        clip_function = k * clip_param + k * (1 - clip_param) * order / agent_num

        # list1 = imp_weight_list[0].data.reshape(1, len(imp_weight_list[0])).cpu().numpy()
        # list2 = imp_weight_list[1].data.reshape(1, len(imp_weight_list[0])).cpu().numpy()
        # g_imp_w = torch.tensor((list1 * list2).reshape(len(imp_weight_list[0]), 1))
        # g_function = torch.clamp(g_imp_w, 1- clip_function/2, 1 + clip_function/2).to(self.config["device"])

        surr1 = imp_weights  * adv_targ_hybrid
        surr2 = torch.clamp(imp_weights , 1.0 - clip_function, 1.0 + clip_function) * adv_targ_hybrid
    ...
    def calculate_advantage(self, buffer):
        ...
        return mean_advantages, advantages, cost_adv

    def train(self, data, imp_weight_list, order, agent_num, logger):
        value_loss, critic_grad_norm, policy_loss, dist_entropy, actor_grad_norm, imp_weights, cost_loss, cost_grad_norm \
                    = self.ppo_update(data, imp_weight_list, order, agent_num)
...
class Runner:
    ...
    def __init__(self,
        ...
        self.buffer = []
        for agent_id in range(self.num_agents):
            tr = MACPAO_Trainer(config, self.policy[agent_id])
            share_observation_space = self.envs.share_observation_space[agent_id]
    ...
    def train(self):
        ...
        old_actions_logprob_dict = {}

        for agent_id in range(self.num_agents):
            mean_adv, adv, cost_adv = self.trainer[agent_id].calculate_advantage(self.buffer[agent_id])
            mean_adv_dict[agent_id] = mean_adv.item()
            adv_dict[agent_id] = adv
            cost_adv_dict[agent_id] = cost_adv

        sequence = []
        sorted_list = sorted(mean_adv_dict.items(), key = lambda kv:(kv[1], kv[0]), reverse=True)
        ...
        for agent_id in sequence:
            action_dim_dict[agent_id] = self.buffer[agent_id].actions.shape[-1]

            self.trainer[agent_id].prep_training()
            self.buffer[agent_id].update_factor(factor)
            available_actions_dict[agent_id] =  None if self.buffer[agent_id].available_actions is None \
            ...
            old_actions_logprob, _ = self.trainer[agent_id].policy.actor.evaluate_actions(
                ...
                available_actions_dict[agent_id],
                self.buffer[agent_id].active_masks[:-1].reshape(-1, *self.buffer[agent_id].active_masks.shape[2:]))
            old_actions_logprob_dict[agent_id] =  old_actions_logprob

        order = 1
        for agent_id in sequence:
            imp_weight_list = []
            for _ in range(self.config["ppo_epoch"]):
                data_generator = self.buffer[agent_id].feed_forward_generator(adv_dict[agent_id], self.config["num_mini_batch"], cost_adv=cost_adv_dict[agent_id])
                for sample in data_generator:
                    data = self.trainer[agent_id].calculate_imp_weights(sample)
                    imp_weight_list.append(data[0])
                self.trainer[agent_id].train(data, imp_weight_list, order, self.num_agents, logger=self.logger)
            order += 1
        ...
        for agent_id in sequence:
            ...
            action_prod = torch.prod(torch.exp(new_actions_logprob.detach()- old_actions_logprob_dict[agent_id].detach()).reshape(self.config["episode_length"],self.config["n_rollout_threads"], action_dim_dict[agent_id]), dim=-1, keepdim=True)
...
if __name__ == '__main__':
    set_np_formatting()
    args, cfg_env, cfg_train = multi_agent_args(algo="macpao")
